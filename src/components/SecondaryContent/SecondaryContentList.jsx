...
class SecondaryContentList extends Component {
    ...
    componentDidMount() {
        ...
        console.log('window height bottom is:', this.props.windowHeight - this.calculateHeight());
    ...
    stopResize = () => {
        if (this.state.isDragging) {
            ...
            if (this.state.initialPos > this.state.onloadPos) {
                //it's going up
                this.setState({ initialPos: this.state.onloadPos + (secondaryRowHeight * 4) });
    ...
    render() {
        ...
        <div style={ renderStyleBottom } className="box3">
            ...
            {/* resizer bar */}
            <div style={{ height: `${containerPullBar}px` }} className="mobileResizer" onMouseDown={(e) => this.startResize(e)} onTouchStart={e => this.startResize(e)}> <div className="resizerIcon"></div> </div>
